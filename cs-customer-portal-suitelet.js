...
define(['N/search', 'N/url', 'N/runtime', 'N/log'], function (search, url, runtime, log) {
    ...
    // filter it to the current customer on filterField, run it. Add a new
    // tab by adding one entry here — no new function needed.
    //
    // NOTE on inTransitTransfers: Transfer Orders have no native customer
    // field. filterField below assumes you've exposed one (a custom body
    // field, or a join back through "Created From" to a Sales Order/customer).
    // If you don't have that, this section will come back empty per customer
    // — see the write-up for options.
    ...
    function searchCustomersFiltered(params) {
        ...
        addDynamicExpression(customerSearch, expression);
    ...
    // Generic runner for one entry in DETAIL_SECTIONS.
    ...
    <div id="gridSection" class="card">
        ...
        <div class="table-wrap" style="margin-top:14px;">
            <table>
                ...
                <tbody id="gridRows">
                    <tr>
                        <td colspan="1" class="loading">Loading customers...</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="card">
            ...
            <div id="openOrdersTab" class="tab-panel active">
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr id="openOrdersHeadRow"></tr></thead>
            ...
            <div id="orderHistoryTab" class="tab-panel">
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr id="orderHistoryHeadRow"></tr></thead>
                        <tbody id="orderHistoryRows"></tbody>
                    </table>
                </div>
            </div>
            ...
            <div id="openBalanceTab" class="tab-panel">
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr id="openBalanceHeadRow"></tr></thead>
                        <tbody id="openBalanceRows"></tbody>
                    </table>
                </div>
            </div>
            ...
            <div id="inTransitTab" class="tab-panel">
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr id="inTransitShipmentsHeadRow"></tr></thead>
                        <tbody id="inTransitShipmentsRows"></tbody>
                    </table>
                </div>
                ...
                <div class="table-wrap">
                    <table>
                        <thead><tr id="inTransitTransfersHeadRow"></tr></thead>
                        <tbody id="inTransitTransfersRows"></tbody>
                    </table>
                </div>
            </div>
        </div>
...
(function () {
    ...
    // Single source of truth for every detail tab. Add a new tab by adding
    // one entry here — loading state, rendering, errors, and CSV export all
    // loop over this instead of being written out per tab.
    var DETAIL_TABS = [
    ...
    function loadFilterOptions() {
        api('filterOptions').then(function (data) {
            document.getElementById('fRep').innerHTML =
                '<option value="">All Reps</option>' + (data.salesReps || []).map(optionHtml).join('');
    ...
    function renderDynamicRows(bodyId, columns, rows, emptyText, clickable) {
        ...
        body.innerHTML = rows.map(function (row) {
            ...
            var dataAttr = clickable ? ' data-customer="' + esc(row.internalId) + '"' : '';
    ...
    function renderTabSection(tab, section) {
        ...
        renderDynamicRows(tab.rows, section.columns || [], section.rows || [], tab.empty, false);
    }
    ...
    function refreshGrid() {
        ...
        document.getElementById('gridRows').innerHTML =
            '<tr><td colspan="1" class="loading">Loading customers...</td></tr>';
        ...
        api('customerSearch', currentFilters()).then(function (data) {
            ...
            renderDynamicRows('gridRows', columns, rows, 'No customers found for selected filters.', true);

            var clickableRows = document.querySelectorAll('#gridRows tr[data-customer]');
    ...
    function renderCustomerDetail(data) {
        ...
        var custName =
            findColumnValue(header, ['companyname']) ||
    ...
    function renderHeaderCards(header) {
        ...
        if (!columns.length) {
            document.getElementById('headerGrid').innerHTML =
                '<div class="loading">No customer header columns found.</div>';
            return;
        }
