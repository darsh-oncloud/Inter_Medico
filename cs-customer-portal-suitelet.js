...
define(['N/search', 'N/url', 'N/runtime', 'N/log'], function (search, url, runtime, log) {
    ...
    const DETAIL_SECTIONS = [
        { key: 'header', paramKey: 'customerHeader', filterField: 'internalid', maxRows: 1, includeTotal: false },
        { key: 'openOrders', paramKey: 'openOrders', filterField: 'entity', maxRows: MAX_DETAIL_ROWS, includeTotal: false },
        { key: 'orderHistory', paramKey: 'orderHistory', filterField: 'entity', maxRows: MAX_DETAIL_ROWS, includeTotal: false },
        { key: 'openBalance', paramKey: 'openBalance', filterField: 'entity', maxRows: MAX_DETAIL_ROWS, includeTotal: false },
        { key: 'inTransitShipments', paramKey: 'inTransitShipments', filterField: 'entity', maxRows: MAX_DETAIL_ROWS, includeTotal: false },
        { key: 'inTransitTransfers', paramKey: 'inTransitTransfers', filterField: 'entity', maxRows: MAX_DETAIL_ROWS, includeTotal: false }
    ];

    // Transaction record types whose "entity" field is a Customer vs a Vendor.
    // Lets the "Name" cell in any dynamic search link to the right record type.
    ...
    function onRequest(context) {
        ...
        try {
            ...
            if (action === 'customerSearch') {
                sendJson(response, searchCustomersFiltered(request.parameters));
                return;
            }

            if (action === 'customerDetail') {
                sendJson(response, getCustomerFullDetail(request.parameters.customerId));
                return;
            }
            ...
            log.error('Customer Portal Error', { action: action, message: e.message, stack: e.stack });
    ...
    function buildRecordUrl(recordType, internalId, isEditMode) {
        ...
        try {
            return url.resolveRecord({ recordType: recordType, recordId: internalId, isEditMode: !!isEditMode });
        } catch (e) {
            log.debug('buildRecordUrl failed', { recordType: recordType, internalId: internalId, message: e.message });
            return '';
        }
    }

    // Generic item view URL. Used instead of url.resolveRecord for item columns because
    // a joined "item" field only gives us an internal id, not the item's exact subtype
    // (inventoryitem / kit / assembly / etc). This NetSuite page auto-redirects to the
    // correct item type page for any given internal id.
    function buildItemUrl(itemId) {
        if (!itemId) {
            return '';
        }

        return '/app/common/item/item.nl?id=' + encodeURIComponent(itemId);
    }
    ...
    function runDynamicSearch(searchObj, maxRows, includeTotal) {
        ...
        searchObj.run().each(function (result) {
            ...
            nsColumns.forEach(function (column, index) {
                ...
                // Capture raw ids (not the display text) for fields we can link to a record.
                if (!column.join && String(column.name || '').toLowerCase() === 'entity') {
                    row.entityId = safeGetValue(result, column);
                } else if (!column.join && String(column.name || '').toLowerCase() === 'item') {
                    row.itemId = safeGetValue(result, column);
                }
            });

            // The row itself (customer, transaction, etc.) can usually be linked directly
            // using its own internal id + record type.
            ...
            if (row.itemId) {
                row.itemUrl = buildItemUrl(row.itemId);
            }
        ...
        return { total: includeTotal ? total : rows.length, columns: columns, rows: rows };
    }
    ...
    function addDynamicExpression(searchObj, expression) {
        ...
        searchObj.filterExpression = (existing && existing.length)
            ? [existing, 'AND', expression]
            : expression;
    ...
    function safeSection(sectionName, callback) {
        try {
            ...
            return { total: 0, columns: [], rows: [], error: e.message || String(e) };
        }
    }
    ...
    function getFilterOptions() {
        return {
            customers: safeSection('Customer Dropdown', getCustomerOptions).rows || [],
            salesReps: safeSection('Sales Rep Dropdown', getSalesRepOptions).rows || [],
    ...
    function getCustomerOptions() {
        const rows = [];
        const customerSearch = loadConfiguredSearch('customerDropdown');
        const columns = customerSearch.columns || [];

        customerSearch.run().each(function (result) {
            const label = columns.length ? getCellValue(result, columns[0]) : '';
            rows.push({ value: result.id, text: label || result.id });
    ...
    function getSalesRepOptions() {
        ...
        repSearch.run().each(function (result) {
            const text = columns.length ? getCellValue(result, columns[0]) : '';
            rows.push({ value: result.id, text: text || result.id });
            return true;
        });
    ...
    function getSubsidiaryOptions() {
        ...
        subsidiarySearch.run().each(function (result) {
            const text = columns.length ? getCellValue(result, columns[0]) : '';
            rows.push({ value: result.id, text: stripHierarchy(text || result.id) });
            return true;
        });
    ...
    function searchCustomersFiltered(params) {
        const customerSearch = loadConfiguredSearch('customerGrid');
        const dynamicFilters = [];

        if (params.customerId) {
            dynamicFilters.push(['internalid', 'anyof', params.customerId]);
        }

        if (params.salesRepId) {
            dynamicFilters.push(['salesrep', 'anyof', params.salesRepId]);
        }
        ...
        if (params.q) {
            dynamicFilters.push([
                ...
                'OR', ['entityid', 'contains', params.q],
                'OR', ['email', 'contains', params.q],
                'OR', ['phone', 'contains', params.q]
            ]);
        }

        addDynamicExpression(customerSearch, buildAndExpression(dynamicFilters));

        const result = runDynamicSearch(customerSearch, MAX_GRID_ROWS, true);
        sortRowsByColumnDesc(result, ['overdue balance', 'overduebalance', 'balance']);
    ...
    function getDetailSection(sectionConfig, customerId) {
        ...
        const filters = [
            [sectionConfig.filterField, 'anyof', customerId]
        ].concat(sectionConfig.extraFilters || []);

        addDynamicExpression(sectionSearch, buildAndExpression(filters));

        return runDynamicSearch(sectionSearch, sectionConfig.maxRows, sectionConfig.includeTotal);
    }

    function emptyDetailResult(customerId) {
        const result = { customerId: customerId || '' };

        DETAIL_SECTIONS.forEach(function (section) {
            result[section.key] = { columns: [], rows: [] };
        });
    ...
    function getCustomerFullDetail(customerId) {
        ...
        DETAIL_SECTIONS.forEach(function (section) {
            result[section.key] = safeSection(section.key, function () {
                return getDetailSection(section, customerId);
            });
        });
...
<div class="page">
    ...
    <div class="topbar">
        <div>
            <div class="eyebrow"><span class="dot"></span>Customer Records</div>
            <h1 class="title">Customer Portal</h1>
            <div class="subtitle">Search any customer, then review open orders, order history, open balance, and in-transit shipments/transfers.</div>
        </div>
    </div>
    ...
    <div id="gridSection" class="card">
        <div class="filters">
            <div>
                <label>Search</label>
                <input id="fQ" placeholder="Company name, ID, email, or phone">
            </div>
            <div>
            ...
            <div>
                ...
                <select id="fSub"><option value="">All Subsidiaries</option></select>
            </div>
            <div>
                <label>&nbsp;</label>
        ...
        <div class="meta">
            ...
            <span>Sorted by highest Overdue Balance first</span>
        </div>

        <div class="table-wrap" style="margin-top:14px;">
            <table>
                <thead><tr id="gridHeadRow"></tr></thead>
                <tbody id="gridRows"><tr><td colspan="1" class="loading">Loading customers...</td></tr></tbody>
            </table>
        </div>
    </div>
    ...
    <div id="detailSection" style="display:none;">
        <div class="card">
            <div class="detail-head">
                <div>
                    ...
                    <div id="custTitle" class="item-title"></div>
                    <div id="custDesc" class="item-desc"></div>
                </div>
                <div class="btn-group">
                    <button id="viewRecordBtn" class="btn" disabled>View Record ↗</button>
                    <button id="exportBtn" class="btn">Export CSV</button>
                    <button id="refreshDetailBtn" class="btn btn-primary">Refresh Customer</button>
                </div>
            </div>
        ...
        <div class="card">
            <div class="tabs">
                <button class="tab-btn active" data-tab="openOrdersTab">Open Orders</button>
                <button class="tab-btn" data-tab="orderHistoryTab">Order History</button>
                <button class="tab-btn" data-tab="openBalanceTab">Open Balance</button>
                <button class="tab-btn" data-tab="inTransitTab">In Transit</button>
            </div>

            <div id="openOrdersTab" class="tab-panel active">
                <div id="openOrdersError"></div>
                <div class="table-wrap">
                    <table><thead><tr id="openOrdersHeadRow"></tr></thead><tbody id="openOrdersRows"></tbody></table>
                </div>
            </div>

            <div id="orderHistoryTab" class="tab-panel">
                <div id="orderHistoryError"></div>
                <div class="table-wrap">
                    <table><thead><tr id="orderHistoryHeadRow"></tr></thead><tbody id="orderHistoryRows"></tbody></table>
                </div>
            </div>

            <div id="openBalanceTab" class="tab-panel">
                <div id="openBalanceError"></div>
                <div class="table-wrap">
                    <table><thead><tr id="openBalanceHeadRow"></tr></thead><tbody id="openBalanceRows"></tbody></table>
                </div>
            </div>

            <div id="inTransitTab" class="tab-panel">
                <div class="subsection-title">Shipments In Transit</div>
                <div id="inTransitShipmentsError"></div>
                <div class="table-wrap">
                    <table><thead><tr id="inTransitShipmentsHeadRow"></tr></thead><tbody id="inTransitShipmentsRows"></tbody></table>
                </div>

                <div class="subsection-title">Transfer Orders In Transit</div>
                <div id="inTransitTransfersError"></div>
                <div class="table-wrap">
                    <table><thead><tr id="inTransitTransfersHeadRow"></tr></thead><tbody id="inTransitTransfersRows"></tbody></table>
                </div>
            </div>
        </div>
    </div>
...
(function () {
    ...
    // Every tab except the header follows the exact same render pattern, so it's
    // driven from this table instead of five near-identical blocks of code.
    var DETAIL_TABS = [
        { key: 'openOrders', label: 'Open Orders', head: 'openOrdersHeadRow', rows: 'openOrdersRows', err: 'openOrdersError', empty: 'No open orders found for this customer.', loading: 'Loading open orders...' },
        { key: 'orderHistory', label: 'Order History', head: 'orderHistoryHeadRow', rows: 'orderHistoryRows', err: 'orderHistoryError', empty: 'No order history found for this customer.', loading: 'Loading order history...' },
        { key: 'openBalance', label: 'Open Balance', head: 'openBalanceHeadRow', rows: 'openBalanceRows', err: 'openBalanceError', empty: 'No open balance found for this customer.', loading: 'Loading open balance...' },
        { key: 'inTransitShipments', label: 'Shipments In Transit', head: 'inTransitShipmentsHeadRow', rows: 'inTransitShipmentsRows', err: 'inTransitShipmentsError', empty: 'No shipments currently in transit for this customer.', loading: 'Loading shipments...' },
        { key: 'inTransitTransfers', label: 'Transfer Orders In Transit', head: 'inTransitTransfersHeadRow', rows: 'inTransitTransfersRows', err: 'inTransitTransfersError', empty: 'No transfer orders currently in transit for this customer.', loading: 'Loading transfer orders...' }
    ];
    ...
    function isNumericLabel(label) {
        ...
        return (
            label.indexOf('quantity') !== -1 || label.indexOf('qty') !== -1 || label.indexOf('amount') !== -1 ||
            label.indexOf('balance') !== -1 || label.indexOf('credit') !== -1 || label.indexOf('cost') !== -1 ||
            label.indexOf('price') !== -1 || label.indexOf('value') !== -1 || label.indexOf('rate') !== -1
        );
    }
    ...
    function showSectionError(id, message) {
        ...
        el.innerHTML = message ? '<div class="section-error">' + esc(message) + '</div>' : '';
    }
    ...
    function api(action, params) {
        ...
        return fetch(finalUrl, { credentials: 'same-origin' }).then(function (response) {
    ...
    function loadFilterOptions() {
        api('filterOptions').then(function (data) {
            document.getElementById('fRep').innerHTML = '<option value="">All Reps</option>' + (data.salesReps || []).map(optionHtml).join('');
            document.getElementById('fSub').innerHTML = '<option value="">All Subsidiaries</option>' + (data.subsidiaries || []).map(optionHtml).join('');
        }).catch(function (e) {
            showError(e.message);
        });
    }
    ...
    function currentFilters() {
        return {
            ...
            salesRepId: document.getElementById('fRep').value,
    ...
    // options:
    //   clickMode: 'customer' -> whole row clickable, data-customer=internalId (Customer grid -> opens the in-app detail panel)
    ...
    //   itemUrlKey:   row property (e.g. 'itemUrl'); if set, only the Item cell becomes a link
    // The url-based keys link a specific cell without making the entire row clickable.
    function renderDynamicRows(bodyId, columns, rows, emptyText, options) {
        options = options || {};

        var clickMode = options.clickMode || 'none';

        var docColumnKey = findColumnKey(columns, options.docUrlKey, function (c) {
        ...
        var entityColumnKey = findColumnKey(columns, options.entityUrlKey, function (c) {
            return !c.join && String(c.name || '').toLowerCase() === 'entity';
        });

        var itemColumnKey = findColumnKey(columns, options.itemUrlKey, function (c) {
            return !c.join && String(c.name || '').toLowerCase() === 'item';
        });
        ...
        body.innerHTML = rows.map(function (row) {
            ...
            if (clickMode === 'customer') {
                trClass = ' class="clickable"';
                dataAttr = ' data-customer="' + esc(row.internalId) + '"';
            }
            ...
            return '<tr' + trClass + dataAttr + '>' + columns.map(function (column) {
                ...
                if (docColumnKey && column.key === docColumnKey && row[options.docUrlKey] && display) {
                    display = linkCell(row[options.docUrlKey], display);
                } else if (entityColumnKey && column.key === entityColumnKey && row[options.entityUrlKey] && display) {
                    display = linkCell(row[options.entityUrlKey], display);
                } else if (itemColumnKey && column.key === itemColumnKey && row[options.itemUrlKey] && display) {
                    display = linkCell(row[options.itemUrlKey], display);
                }
    ...
    function renderTabSection(tab, section) {
        section = section || {};

        showSectionError(tab.err, section.error || '');
        renderHead(tab.head, section.columns || []);

        renderDynamicRows(
            tab.rows, section.columns || [], section.rows || [], tab.empty,
            { docUrlKey: 'viewUrl', entityUrlKey: 'entityUrl', itemUrlKey: 'itemUrl' }
        );
    }
    ...
    function refreshGrid() {
        ...
        document.getElementById('gridRows').innerHTML = '<tr><td colspan="1" class="loading">Loading customers...</td></tr>';

        api('customerSearch', currentFilters()).then(function (data) {
            ...
            renderDynamicRows('gridRows', columns, rows, 'No customers found for selected filters.', { clickMode: 'customer' });

            document.querySelectorAll('#gridRows tr[data-customer]').forEach(function (row) {
                row.addEventListener('click', function () {
                    openCustomer(row.getAttribute('data-customer'));
                });
            });
            ...
            document.getElementById('gridRows').innerHTML = emptyRow(1, 'Unable to load customer results.');
        });
    }
    ...
    function openCustomer(customerId) {
        ...
        document.getElementById('custTitle').innerHTML = 'Loading customer...';
        document.getElementById('custDesc').innerHTML = '';
        document.getElementById('headerGrid').innerHTML = '<div class="loading">Loading customer header...</div>';
        document.getElementById('viewRecordBtn').disabled = true;

        showSectionError('headerError', '');

        DETAIL_TABS.forEach(function (tab) {
            showSectionError(tab.err, '');
            document.getElementById(tab.head).innerHTML = '<th>Loading</th>';
            document.getElementById(tab.rows).innerHTML = emptyRow(1, tab.loading);
        });

        api('customerDetail', { customerId: customerId }).then(function (data) {
            currentDetailData = data;
            renderCustomerDetail(data);
        }).catch(function (e) {
            showError(e.message);
            document.getElementById('custTitle').innerHTML = 'Unable to load customer';
        });
    }
    ...
    function renderCustomerDetail(data) {
        ...
        var custName = findColumnValue(header, ['companyname']) || findColumnValue(header, ['entityid']) || firstNonEmptyValue(header) || 'Customer Detail';
        var email = findColumnValue(header, ['email']) || '';
        var phone = findColumnValue(header, ['phone']) || '';
        var overdueBalance = findColumnValue(header, ['overdue balance', 'overduebalance']) || '';
        var inactiveValue = findColumnValue(header, ['isinactive', 'inactive']) || '';

        var isInactive = inactiveValue === true || inactiveValue === 'T' ||
            String(inactiveValue).toLowerCase() === 'true' || String(inactiveValue).toLowerCase() === 'yes';

        var hasOverdue = Number(String(overdueBalance).replace(/,/g, '')) > 0;

        document.getElementById('custTitle').innerHTML =
            esc(custName) +
            ' <span class="badge ' + (!isInactive ? 'active' : '') + '">' + esc(isInactive ? 'Inactive' : 'Active') + '</span>' +
            (hasOverdue ? ' <span class="badge warn">Overdue Balance</span>' : '');
        ...
        // (computed server-side from the customer's own internal id + record type).
        currentCustomerViewUrl = ((header.rows || [])[0] || {}).viewUrl || '';
        document.getElementById('viewRecordBtn').disabled = !currentCustomerViewUrl;

        showSectionError('headerError', header.error || '');
        renderHeaderCards(header);

        DETAIL_TABS.forEach(function (tab) {
            renderTabSection(tab, data[tab.key]);
        });
    }
    ...
    function renderHeaderCards(header) {
        ...
        if (!columns.length) {
            document.getElementById('headerGrid').innerHTML = '<div class="loading">No customer header columns found.</div>';
            return;
        }

        document.getElementById('headerGrid').innerHTML = columns.map(function (column) {
            var value = row[column.key];
            return '<div class="kv"><div class="k">' + esc(column.label) + '</div><div class="v">' + formatValue(value, column) + '</div></div>';
        }).join('');
    }
    ...
    function toCsvValue(value) {
        ...
        // Backslashes are doubled because this JS is inside the outer SuiteScript template literal.
        if (/[",\\n\\r]/.test(display)) {
    ...
    function sectionToCsvLines(title, section) {
        ...
        lines.push(columns.map(function (c) { return toCsvValue(c.label); }).join(','));

        rows.forEach(function (row) {
            lines.push(columns.map(function (c) { return toCsvValue(row[c.key]); }).join(','));
        });
    ...
    function exportCustomerDetailCsv() {
        if (!currentDetailData) {
            return;
        }

        var lines = sectionToCsvLines('Customer Header', currentDetailData.header);

        DETAIL_TABS.forEach(function (tab) {
            lines = lines.concat(sectionToCsvLines(tab.label, currentDetailData[tab.key]));
        });

        // Double backslashes preserve \r\n inside the generated browser JS.
        var csv = lines.join('\\r\\n');
        ...
        link.href = blobUrl;
        link.download = 'customer-' + (currentCustomerId || 'detail') + '.csv';
    ...
    document.getElementById('viewRecordBtn').addEventListener('click', function () {
        if (currentCustomerViewUrl) {
            window.open(currentCustomerViewUrl, '_blank', 'noopener');
        }
    });

    document.getElementById('exportBtn').addEventListener('click', exportCustomerDetailCsv);

    document.getElementById('fRep').addEventListener('change', refreshGrid);
